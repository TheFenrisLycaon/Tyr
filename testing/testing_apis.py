...
from constants.constants import TASK, USER
from controllers.main import app as tst_app
from models.models import (Event, Goal, Habit, HabitDay, Project, Quote,
                           Readable, Snapshot, Task)
...
class APITestCase(BaseTestCase):
    ...
    def test_user_calls(self):
        ...
        response = self.post_json(
            "/api/user/me", {"timezone": TZ, "birthday": DOB}, headers=self.api_headers
        )
        u = response.get("user")
    ...
    def test_habit_calls(self):
        ...
        # Update
        response = self.post_json(
            "/api/habit", {"id": h.get("id"), "name": "Walk"}, headers=self.api_headers
        )
        ...
        for act in actions:
            ...
            response = self.post_json(
                "/api/habit/%s" % act.get("action"), params, headers=self.api_headers
            )
        ...
        response = self.get_json(
            "/api/habit/recent", {"days": 3}, headers=self.api_headers
        )
        ...
        # Delete
        response = self.post_json(
            "/api/habit/delete", {"id": h.get("id")}, headers=self.api_headers
        )
    ...
    def test_goal_calls(self):
        ...
        params = {
            "id": goal.get("id"),
            "text": json.dumps(["New goal 1", "New goal 2 with unicode. ありがとう"]),
        }
        ...
        self.assertEqual(goal.get("text")[1], "New goal 2 with unicode. ありがとう")
    ...
    def test_task_calls(self):
        ...
        response = self.post_json(
            "/api/task",
            {"id": h.get("id"), "title": "Dont forget the sugar", "status": TASK.DONE},
        ...
        # Archive complete
        response = self.post_json(
            "/api/task/action", {"action": "archive_complete"}, headers=self.api_headers
        )
        ...
        # Delete
        response = self.post_json(
            "/api/task/delete", {"id": h.get("id")}, headers=self.api_headers
        )
    ...
    def test_project_calls(self):
        ...
        p.Update(
            urls=["http://www.x.com", "http://www.y.com"],
            title="New Project",
            subhead="Details",
        )
        p.put()
        ...
        # Delete
        response = self.post_json(
            "/api/project/delete", {"id": prj.get("id")}, headers=self.api_headers
        )
    ...
    def test_event_calls(self):
        ...
        # Delete
        response = self.post_json(
            "/api/event/delete", {"id": h.get("id")}, headers=self.api_headers
        )
    ...
    def test_readable_calls(self):
        ...
        # Search
        response = self.get_json(
            "/api/readable/search", {"term": "clark"}, headers=self.api_headers
        )
        ...
        # Delete
        response = self.post_json(
            "/api/readable/delete", {"id": r.get("id")}, headers=self.api_headers
        )
    ...
    def test_quote_calls(self):
        ...
        # Search
        response = self.get_json(
            "/api/quote/search", {"term": "think"}, headers=self.api_headers
        )
    ...
    def test_journal_calls(self):
        # Create / Submit
        params = {"data": json.dumps({"metric1": 10})}
        response = self.post_json(
            "/api/journal/submit", params, headers=self.api_headers
        )
        jrnl = response.get("journal")
        self.assertIsNotNone(jrnl)

        # Update
        params = {"id": jrnl.get("id"), "data": json.dumps({"metric1": 20})}
    ...
    def test_tracking_calls(self):
        ...
        response = self.post_json(
            "/api/tracking",
            {"date": DATE, "data": json.dumps({"foo": "bar"})},
            headers=self.api_headers,
        )
    ...
    def test_tyrapp_agent_api(self):
        response = self.post_json(
            "/api/agent/tyrapp/request", {"message": "hi"}, headers=self.api_headers
        )
