...
class UserSearchable(ndb.Model):
    ...
    def valid_atom_value(self, af, val):
        ...
        if not valid:
            logging.debug(
                "Invalid %s atom field '%s' of size %d"
                % (self._get_kind(), af, val_size)
            )
        return valid

    def doc_from_fields(
        self, text_fields=None, atom_fields=None, repeated_attom_fields=None
    ):
        fields = []
        if text_fields:
    ...
    def update_sd(self, delete=False, index_put=True):
        ...
        try:
            ...
            logging.warning("Search Index Error when updating search doc: %s" % e)
            return (None, None)
    ...
    def Search(cls, user, term, limit=20):
        ...
        try:
            ...
            keys = [
                ndb.Key(kind, sd.doc_id, parent=user.key)
...
class User(ndb.Model):
    ...
    def SyncActive(sync_integration_id, limit=200):
        ...
        return [
            u for u in User.query().filter(fltr).fetch(limit=limit) if not u.inactive()
        ]

    @staticmethod
    def Create(email=None, g_id=None, name=None, password=None):
        from constants.constants import (ADMIN_EMAIL, APP_OWNER,
                                         DEFAULT_USER_SETTINGS, SENDER_EMAIL,
                                         SITENAME)
    ...
    def get(self, cls, id=None, str_id=False):
        ...
        """
        if id:
            if isinstance(id, str) and id.isdigit() and not str_id:
                id = int(id)
    ...
    def get_setting_prop(self, path, default=None):
        ...
        if settings:
            ...
            for i, pi in enumerate(path):
                ...
                last = i == len(path) - 1
                if last:
                    empty = isinstance(cursor, dict) and len(list(cursor.keys())) == 0
                    if empty:
                        return default
                    else:
    ...
    def aes_token(self, client_id="google", add_props=None):
        from utils.aes_cypher import AESCipher

        cypher = AESCipher(secrets.AES_CYPHER_KEY)
        data = {"client_id": client_id, "user_id": self.key.id()}
        if add_props:
...
class Project(ndb.Model):
    ...
    progress_ts = ndb.IntegerProperty(
        repeated=True, indexed=False
    )  # Timestamp (ms) for each progress step (len 10)
    ...
    def Active(user):
        return (
            Project.query(ancestor=user.key)
            .filter(Project.archived == False)
            .order(Project.starred)
            .order(-Project.dt_created)
            .fetch(limit=20)
        )
...
class Task(ndb.Model):
    ...
    timer_target_ms = ndb.IntegerProperty(
        indexed=False, default=0
    )  # For current timer run
    ...
    def json(self, references=["project"]):
        res = {
            ...
            "timer_last_start": tools.unixtime(self.timer_last_start)
            if self.timer_last_start
            else 0,
        }
        if references:
    ...
    def CountCompletedSince(user, since):
        return (
            Task.query(ancestor=user.key)
            .order(-Task.dt_done)
            .filter(Task.dt_done > since)
            .count(limit=None)
        )
    ...
    def Open(user, limit=10):
        return (
            Task.query(ancestor=user.key)
            .filter(Task.status == TASK.NOT_DONE)
            .order(-Task.dt_created)
            .fetch(limit=limit)
        )

    @staticmethod
    def Recent(
        user, limit=10, offset=0, with_archived=False, project_id=None, prefetch=None
    ):
        ...
        if project_id:
            q = q.filter(
                Task.project == ndb.Key("User", user.key.id(), "Project", project_id)
            )
    ...
    def Create(user, title, due=None, tomorrow=None):
        if not due:
            ...
            same_day_hour = tools.safe_number(
                task_prefs.get("same_day_hour", 16), default=16, integer=True
            )
            due_hour = tools.safe_number(
                task_prefs.get("due_hour", 22), default=22, integer=True
            )
    ...
    def Update(self, **params):
        from constants.constants import TASK_DONE_REPLIES
        ...
        if "project_id" in params:
            if params["project_id"]:
                self.project = ndb.Key(
                    "User", self.key.parent().id(), "Project", params.get("project_id")
                )
            else:
                self.project = None
...
class Habit(ndb.Model):
    ...
    def Active(user):
        return (
            Habit.query(ancestor=user.key)
            .filter(Habit.archived == False)
            .fetch(limit=HABIT.ACTIVE_LIMIT)
        )
    ...
    def delete_history(self):
        ...
        """
        from controllers.tasks import backgroundHabitDayDeletion

        tools.safe_add_task(
            backgroundHabitDayDeletion,
            self.key.urlsafe(),
            _queue="background-deletion-queue",
        )
...
class HabitDay(ndb.Model):
    ...
    def All(habit_key, limit=100, keys_only=True):
        ...
        return (
            HabitDay.query(ancestor=user_key)
            .filter(HabitDay.habit == habit_key)
    ...
    def GetOrInsert(habit, date):
        ...
        hd = HabitDay.get_or_insert(
            id, habit=habit.key, date=date, parent=habit.key.parent()
        )
        return hd
...
class JournalTag(ndb.Model):
    ...
    def json(self):
        return {"id": self.key.id(), "name": self.name, "type": self.type}
        ...
        if text and isinstance(text, str):
            ...
            for existing_tag, key in zip(existing_tags, people_ids + hashtag_ids):
                if not existing_tag:
                    prefix = key.id()[0]
                    tag_type = (
                        JOURNALTAG.HASHTAG if prefix == "#" else JOURNALTAG.PERSON
                    )
                    jt = JournalTag(
                        id=key.id(), name=key.id()[1:], type=tag_type, parent=user.key
                    )
                    new_jts.append(jt)
                    all_jts.append(jt)
                else:
...
class MiniJournal(ndb.Model):
    ...
    def json(self):
        ...
        if self.location:
            res.update({"lat": self.location.lat, "lon": self.location.lon})
        return res
    ...
    def parse_tags(self):
        ...
        questions = (
            tools.getJson(user.settings, {}).get("journals", {}).get("questions", [])
        )
...
class Snapshot(ndb.Model):
    ...
    def Recent(user, limit=500):
        return (
            Snapshot.query(ancestor=user.key)
            .order(-Snapshot.dt_created)
            .fetch(limit=limit)
        )
...
class Event(ndb.Model):
    ...
    @staticmethod
    def Create(
        user, date_start, date_end=None, title=None, details=None, color=None, **params
    ):
        if not date_end:
            date_end = date_start
        return Event(
...
class Goal(ndb.Model):
    ...
    assessments = ndb.IntegerProperty(
        indexed=False, repeated=True
    )  # 1-5 rating for each goal
    ...
    def Year(user, year, with_annual=False):
        ...
        sorted_goals = sorted(
            [
                g
                for g in goals
                if g.date.year == year and (with_annual or not g.annual())
            ],
            key=lambda g: g.date,
        )
        return sorted_goals
    ...
    def Current(user, which="all"):
        ...
        if which in ["all", "month"]:
            monthly_id = ndb.Key(
                "Goal", datetime.strftime(date, "%Y-%m"), parent=user.key
            )
            keys.append(monthly_id)
        if which in ["all", "longterm"]:
            longterm_id = ndb.Key(
                "Goal", datetime.strftime(date, "longterm"), parent=user.key
            )
...
class TrackingDay(ndb.Model):
    ...
    def Range(user, dt_start, dt_end, offset=0, limit=30):
        return (
            TrackingDay.query(ancestor=user.key)
            .order(-TrackingDay.date)
...
class Readable(UserSearchable):
    ...
    def generate_sd(self):
        return self.doc_from_fields(
            text_fields=["title", "notes", "author"],
    ...
    def lookup_readable(self, user):
        ...
        if self.source:
            if USE_FTS:
                ...
                lookup_title_quoted = (
                    '"%s"'
                    % re.sub(r"\((.*)\)$", "", self.source.replace('"', '\\"')).strip()
                )
...
class Report(ndb.Model):
    ...
    def Fetch(user, limit=50):
        return (
            Report.query(ancestor=user.key).order(-Report.dt_created).fetch(limit=limit)
        )

    @staticmethod
    def Create(
        user, title="Unnamed Report", type=REPORT.HABIT_REPORT, specs=None, ftype=None
    ):
    ...
    def print_type(self):
        return REPORT.TYPE_LABELS.get(self.type)

    def print_status(self):
        return REPORT.STATUS_LABELS.get(self.status)
    ...
    def run(self, start_cursor=None):
        ...
        from controllers.reports import (EventReportWorker, GoalReportWorker,
                                         HabitReportWorker,
                                         JournalReportWorker,
                                         ProjectReportWorker, TaskReportWorker,
                                         TrackingReportWorker)
        ...
        if worker_class:
            ...
            if worker and self.status not in [REPORT.ERROR, REPORT.CANCELLED]:
                ...
                logging.error(
                    "Worker not created or invalid status for run(): type %d"
                    % self.type
                )
