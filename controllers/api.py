...
class ProjectAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        self.set_response({"projects": [p.json() for p in projects]}, success=True)
...
class TaskAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        self.set_response(
            {"tasks": [t.json(references=["project"]) for t in tasks]}, success=True
        )
...
class HabitAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        self.set_response({"habits": [habit.json() for habit in habits]}, success=True)
        ...
        self.set_response(
            {
                ...
                "habitdays": tools.lookupDict(
                    habitdays, keyprop="key_id", valueTransform=lambda hd: hd.json()
                ),
            }
        )
    ...
    def range(self, d):
        ...
        habits = Habit.Active(self.user)
        habitdays = HabitDay.Range(
            self.user,
            habits,
            tools.fromISODate(start),
            until_date=tools.fromISODate(end),
        )
        self.set_response(
            {
                "habits": [habit.json() for habit in habits],
    ...
    def increment(self, d):
        ...
        hd = None
        if habit:
            marked_done, hd = HabitDay.Increment(
                habit, tools.fromISODate(day_iso), cancel=cancel
            )
            if marked_done:
                self.message = random.choice(HABIT_DONE_REPLIES)
            else:
    ...
    def update(self, d):
        ...
        self.set_response({"habit": habit.json() if habit else None})
...
class GoalAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        self.set_response({"goals": [goal.json() for goal in goals]}, success=True)
    ...
    def update(self, d):
        ...
        self.set_response({"goal": goal.json() if goal else None})
...
class EventAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        self.set_response(
            {"events": [event.json() for event in events]}, debug=True, success=True
        )
    ...
    def update(self, d):
        ...
        self.set_response({"event": event.json() if event else None})
    ...
    def batch_create(self, d):
        ...
        if isinstance(events, list):
            for e in events:
                if "date_start" in e and isinstance(e["date_start"], str):
                    e["date_start"] = tools.fromISODate(e["date_start"])
                if "date_end" in e and isinstance(e["date_end"], str):
                    e["date_end"] = (
                        tools.fromISODate(e["date_end"])
                        if e.get("date_end")
                        else e.get("date_start")
                    )
...
class ReadableAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        self.set_response({"readables": [r.json() for r in readables]}, success=True)
            ...
            if r.source == "pocket":
                ...
                if access_token:
                    ...
                    if params.get("favorite") == 1:
                        pocket.update_article(
                            access_token, r.source_id, action="favorite"
                        )
                    if params.get("read") == 1:
                        pocket.update_article(
                            access_token, r.source_id, action="archive"
                        )
            r.put()
            self.success = True
    ...
    def random_batch(self, d):
        ...
        """
        BATCH_SIZE = 50
        sample_keys = Readable.Fetch(
            self.user, with_notes=True, limit=500, keys_only=True
        )
    ...
    def search(self, d):
        ...
        data = {"readables": [r.json() for r in readables if r]}
...
class QuoteAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        quotes = Quote.Fetch(
            self.user, readable_id=readable_id, limit=max, offset=offset
        )
    ...
    def batch_create(self, d):
        ...
        dbp = []
        for q in quotes:
            if "dt_added" in q and isinstance(q["dt_added"], str):
    ...
    def search(self, d):
        ...
        data = {"quotes": [q.json() for q in quotes if q]}
        self.set_response(data)
        ...
        if quote:
            if action == "link_readable":
                ...
                if readable:
                    ...
                    self.message = (
                        "Couldn't match readable to quote source - %s" % quote.source
                    )
            else:
...
class JournalTagAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        self.set_response({"tags": [tag.json() for tag in tags]}, success=True)
...
class JournalAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        self.set_response({"journals": [j.json() for j in journals if j]}, success=True)
    ...
    def today(self, d):
        ...
        self.set_response({"journal": jrnl.json() if jrnl else None}, success=True)

    @auth.role("user")
    def update(self, d):
        """ """
        id = self.request.get("id")
        params = tools.gets(self, strings=["tags_from_text"], json=["data"])
        jrnl = None
        if id:
...
class SnapshotAPI(handlers.JsonRequestHandler):
    ...
    def submit(self, d):
        ...
        self.set_response({"snapshot": snap.json() if snap else None}, debug=True)
...
class TrackingAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        tracking_days = TrackingDay.Range(
            self.user, date_from, date_to, limit=max, offset=offset
        )
        self.set_response(
...
class UserAPI(handlers.JsonRequestHandler):
    ...
    def update_self(self, d):
        ...
        self.success = True
        self.set_response({"message": message, "user": self.user.json()}, debug=True)
...
class AuthenticationAPI(handlers.JsonRequestHandler):
    def google_login(self):
        ...
        if ok:
            ...
            if not u:
                self.message = (
                    "tyr is no longer accepting new signups for the hosted version"
                )
    ...
    def google_auth(self):
        ...
        redir_url = user = None
        if client_id == "google":
            # Part of Google Home / API.AI auth tyr
            if (
                redirect_uri
                == "https://oauth-redirect.googleusercontent.com/r/%s"
                ...
                if not user:
                    ...
                    if ok:
                        user = User.GetByEmail(
                            _email, create_if_missing=True, name=name
                        )
                if user:
                    ...
                    redir_url += urllib.parse.urlencode(
                        {
    ...
    def _google_return_tokens(self, user):
        expire_secs = 60 * 60
        access_token = user.aes_token(
            client_id="google",
            add_props={
                "token_type": "access",
                "expiration_time": tools.sdatetime(
                    datetime.now() + timedelta(minutes=expire_secs / 60)
                ),
            },
        )
        refresh_token = user.aes_token(
            client_id="google", add_props={"token_type": "refresh"}
        )
        self.set_response(
            {
                "token_type": "bearer",
    ...
    def google_token(self):
        ...
        https://developers.google.com/actions/identity/oauth2-assertion-tyr
        """
        ...
        assertion_tyr = grant_type == "urn:ietf:params:oauth:grant-type:jwt-bearer"
        email = None
        ok = False
        if assertion_tyr:
            decoded = jwt.decode(
            ...
            if decoded:
                ...
                if iss_ok:
                    ...
                    if intent == "get":
                        ...
                        u = User.Create(
                            email=email, g_id=g_id, name=decoded.get("name")
                        )
                        if u:
                            self._google_return_tokens(u)
                            ok = True
        ...
        if not ok and email:
            ...
            self.set_response({"error": "linking_error", "login_hint": email})
    ...
    def google_service_authenticate(self, service_name, d):
        ...
        if service_name == "fit":
            ...
            from services.tyr_bigquery import BigQueryClient
    ...
    def validate_google_id_token(self, token):
        ...
        if g_response.status_code == 200:
            ...
            if "aud" in json_response:
                ...
                client_id = (
                    secrets.DEV_GOOGLE_CLIENT_ID
                    if tools.on_dev_server()
                    else secrets.GOOGLE_CLIENT_ID
                )
                if aud == client_id:
                    success = True
...
class AnalysisAPI(handlers.JsonRequestHandler):
    ...
    def get(self, d):
        ...
        if with_tasks:
            tasks = Task.DueInRange(
                self.user, dt_start, dt_end + timedelta(days=1), limit=100
            )
        self.set_response(
            {
                "dates": iso_dates,
...
class IntegrationsAPI(handlers.JsonRequestHandler):
    ...
    def update_integration_settings(self, d):
        ...
        self.set_response({"user": self.user.json()}, success=True)

    @auth.role("user")
    def goodreads_shelf(self, d):
        from services import goodreads

        self.success, readables = goodreads.get_books_on_shelf(
            self.user, shelf="currently-reading"
        )
        if not self.success:
            self.message = "There was a problem - please make sure you've entered your Goodreads ID on the integrations page"
        self.set_response({"readables": [r.json() for r in readables]})
    ...
    def pocket_sync(self, d):
        ...
        readables = []
        if access_token:
            self.success, readables, latest_timestamp = pocket.sync(
    ...
    def pocket_authenticate(self, d):
        ...
        if code:
            ...
            self.success = True
        self.set_response({"redirect": redirect})
    ...
    def pocket_authorize(self, d):
        ...
        if access_token:
            ...
            self.update_session_user(self.user)
            self.success = True
        self.set_response({"user": self.user.json() if self.user else None})
    ...
    def pocket_disconnect(self, d):
        ...
        self.update_session_user(self.user)
        self.set_response(
            {"user": self.user.json() if self.user else None}, success=True
        )
    ...
    def evernote_authenticate(self, d):
        ...
        Step 1
        """
        from services import tyr_evernote

        authorize_url = tyr_evernote.get_request_token(
            self.user, self.request.host_url + "/app/integrations/evernote_connect"
        )
        self.success = bool(authorize_url)
        self.set_response(data={"redirect": authorize_url}, debug=True)
    ...
    def evernote_authorize(self, d):
        ...
        access_token, en_user_id = tyr_evernote.get_access_token(
            self.user, ot, verifier
        )
        ...
        self.set_response(data={"user": self.user.json()}, debug=True)
        ...
        if reason in ENABLED_REASONS and memcache.get(mc_key) != "ignore":
            ...
            if user:
                config_notebook_ids = user.get_integration_prop(
                    "evernote_notebook_ids", default=""
                ).split(
                    ","
                )  # Comma sep
                if not config_notebook_ids or notebook_guid in config_notebook_ids:
                    uid, title, content, url = tyr_evernote.get_note(user, note_guid)
                    if title and content:
                        max_quote_length = user.get_integration_prop(
                            "evernote_max_quote_length", 1200
                        )
                        ...
                        if is_article:
                            ...
                            r = Readable.CreateOrUpdate(
                                user, uid, source="evernote", url=url, title=title
                            )
                            r.put()
                            self.success = True
                        else:
                    ...
                    memcache.set(
                        mc_key, "ignore", time=60 * 60 * 48
                    )  # remember ignored for 12 hrs
            else:
                logging.warning("User not found")
        else:
            logging.debug(
                "Ignoring, reason: %s not enabled, or notebook ignored" % reason
            )
...
class AgentAPI(handlers.JsonRequestHandler):
    @auth.role("admin")
    def spoof(self, d):
        from services.agent import AGENT_FBOOK_MESSENGER, ConversationAgent
    ...
    def _get_agent_type(self, body):
        ...
        from services.agent import AGENT_FBOOK_MESSENGER, AGENT_GOOGLE_ASST

        originalRequest = body.get("originalRequest", {})
        source = originalRequest.get("source")
        if source:
            return {"google": AGENT_GOOGLE_ASST, "facebook": AGENT_FBOOK_MESSENGER}.get(
                source
            )
    ...
    def apiai_request(self, d):
        ...
        if auth_key == secrets.API_AI_AUTH_KEY:
            ...
            if action == "input.disconnect":
                speech = "Alright, you've disconnected your tyr account"
                end_convo = True
                ...
                speech, data, end_convo = ca.respond_to_action(
        ...
        data["google"] = {"expectUserResponse": not end_convo}
        res["data"] = data
        res["contextOut"] = []
    ...
    @auth.role("user")
    def tyrapp_request(self, d):
        from services.agent import AGENT_tyr_APP, ConversationAgent

        ca = ConversationAgent(type=AGENT_tyr_APP, user=self.user)
        message = self.request.get("message")
        action, params = ca.parse_message(message)
        speech, data, end_convo = ca.respond_to_action(action, parameters=params)
        data = {"reply": speech}
...
class ReportAPI(handlers.JsonRequestHandler):
    ...
    def list(self, d):
        ...
        data = {"reports": [r.json() for r in reports]}
    ...
    def generate(self, d):
        ...
        report.put()
        tools.safe_add_task(
            backgroundReportRun, report.key.urlsafe(), _queue="report-queue"
        )
        self.set_response(
            success=True,
            message="%s generating..." % report.title,
            data={"report": report.json() if report else None},
        )
            ...
            if r.is_done() and r.gcs_files:
                ...
                if tools.on_dev_server():
                    try:
                        ...
                        self.response.headers["Content-Type"] = Report.content_type(
                            r.extension
                        )
                ...
                self.set_response(
                    success=False, status=404, message="Report not ready"
                )  # Not found
        else:
