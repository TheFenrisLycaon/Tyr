...
from models.models import HabitDay, TrackingDay, User
...
class SyncReadables(handlers.BaseRequestHandler):
    def get(self):
        from services import goodreads, pocket
...
class SyncFromGoogleFit(handlers.BaseRequestHandler):
    def get(self):
        ...
        for user in users:
            ...
            logging.debug(
                "Running SyncFromGoogleFit cron for %s on %s..." % (user, date)
            )
            if fit_enabled:
...
class PushToBigQuery(handlers.BaseRequestHandler):
    def get(self):
        ...
        date = (datetime.today() - timedelta(days=1)).date()
        for user in users:
            enabled = bool(user.get_integration_prop("bigquery_dataset_name")) and bool(
                user.get_integration_prop("bigquery_table_name")
            )
            if enabled:
                logging.debug(
                    "Running PushToBigQuery cron for %s on %s..." % (user, date)
                )
                bq_client = BigQueryClient(
                    user, days_ago=days_ago, days_ago_end=days_ago_end
                )
                if bq_client:
                    bq_client.run()
            else:
...
class DeleteOldReports(handlers.BaseRequestHandler):
    def get(self):
        ...
        old_reports = (
            Report.query().filter(Report.dt_created < cutoff).fetch(limit=None)
        )
        n = 0
        if old_reports:
